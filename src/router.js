...
import {businessCoreState} from "./business-core.js";
...
export default {
  async fetch(request,env,ctx){
    ...
    if(url.pathname==='/api/health'){const bc=businessCoreState(env);return new Response(JSON.stringify({ok:true,worker:'package-tracking',d1Bound:Boolean(env.TRACKING_DB),assetsBound:Boolean(env.ASSETS),ttgAuthBound:Boolean(env.TTG_AUTH),hunterConfigured:Boolean(env.HUNTER_API_URL),businessCoreMentioned:bc.mentioned,businessCoreConfigured:bc.configured,businessCoreActive:bc.active,businessCoreStaged:bc.staged,businessCoreRequired:bc.required,opsApi:true,opsPrivateOwnerRecovery:true,standaloneOpsUi:false,adminOperationsTarget:ADMIN_ORIGIN,docOpsConnect:true}),{status:200,headers:JSON_HEADERS});}
